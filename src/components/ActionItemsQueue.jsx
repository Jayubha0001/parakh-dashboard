...
// Renders one "weak vs state average / weak band" list — used for both
// weak SUBJECTS and weak GRADES (classes), which share the exact same data
// shape, so both get identical styling and logic instead of two
// hand-maintained copies that could drift apart.
const WeakItemsBlock = ({ items, noun, icon, district, dualSemester }) => {
    ...
    return (
      <Box sx={{ mt: 2.5, p: 2.5, borderRadius: 2, bgcolor: "#EEF7EE", border: "1px solid #C9E6C9" }}>
        <Typography sx={{ fontSize: 13.5, color: "#1B5E20", fontWeight: 600 }}>
          ✅ No weak {noun.toLowerCase()} found — {district} is at or above{" "}
          {dualSemester ? "the state average, in both semesters," : "the relevant line"} on every {noun.toLowerCase().replace(/s$/, "")}.
        </Typography>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ mt: 2.5, p: 2.5, borderRadius: 2, bgcolor: "#FFFBEF", border: "1px dashed #F0B429" }}>
      ...
      <Typography sx={{ fontSize: 12.5, color: "text.secondary", mb: 1.5 }}>
        {dualSemester
          ? `Every ${noun.toLowerCase().replace(/s$/, "")} below is behind that semester's own state average — Sem 1 and Sem 2 are compared separately, in order from furthest behind to closest.`
          : items[0]?.stateAvg != null
          ? `Every ${noun.toLowerCase().replace(/s$/, "")} below is behind the state average, in order from furthest behind to closest.`
          : "Every item below is under the 60% Strong line — Watch (45–59%) and Needs Support (<45%) both included, in order from weakest to least-weak."}
      </Typography>
